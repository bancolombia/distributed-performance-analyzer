defmodule Perf.MetricsAnalyzer do
  ...
  def handle_cast(:compute, %ExecutionModel{duration: duration}) do
    ...
    curve = Enum.map(steps, fn step ->
      ...
      {step, throughput, partial.concurrency, mean_latency, partial.success_max_latency, mean_latency_http, partial}
    end)
    ...
    Enum.each(sorted_curve, fn {step, throughput, concurrency, lat_total, max_latency, mean_latency_http, partial} ->
      IO.puts("#{concurrency}, #{round(throughput)} -- #{round(lat_total)}ms -- #{round(max_latency)}ms, #{round(mean_latency_http)}ms, #{partial.fail_http_count}, #{partial.protocol_error_count}, #{partial.error_conn_count}, #{partial.nil_conn_count}")
    end)
