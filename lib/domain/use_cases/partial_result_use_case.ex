defmodule DistributedPerformanceAnalyzer.Domain.UseCase.PartialResultUseCase do
  ...
  def calculate(result_list, opts) do
    Enum.reduce(result_list, PartialResult.new(concurrency: 1), fn item, acc -> calculate(acc, item, opts[:keep_responses]) end)
  end
  ...
  defp calculate(partial, {_time, {{:fail_http, _status_code}, %{elapsed: elapsed} = request_result}},
         keep_responses) do
    %{partial |
      total_count: partial.total_count + 1,
      http_count: partial.http_count + 1,
